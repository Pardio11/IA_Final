...
def process_and_detect_main_car(source_dir, output_dir, yolo_weights, yolo_cfg, coco_names, image_size=(80, 80)):
    ...
    existing_files = [f for f in os.listdir(output_dir) if f.startswith("SSR") and f.endswith(".jpg")]
    if existing_files:
        last_index = max([int(re.search(r"SSR_(\d+)", f).group(1)) for f in existing_files])
    else:
    ...
    for file_name in os.listdir(source_dir):
        ...
        if car_detections:
            ...
            for angle in range(0, 360, 90):
                ...
                frame_name = os.path.join(output_dir, f"SSR_{frame_count:05d}.jpg")
...
# Ejemplo de uso
source_dir = r"src\Ultimate\SSR"  # Carpeta de imágenes de origen
output_dir = r"src\UltiCopy\SSR"  # Carpeta de salida
